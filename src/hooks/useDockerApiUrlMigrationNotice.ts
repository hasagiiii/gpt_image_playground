...
import { getRuntimeConfig } from '../lib/runtimeEnv'
...
export function useDockerApiUrlMigrationNotice() {
  ...
  useEffect(() => {
    if (getRuntimeConfig('DOCKER_DEPLOYMENT') !== 'true') return
    if (getRuntimeConfig('DOCKER_LEGACY_API_URL_USED') !== 'true') return
