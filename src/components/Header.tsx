...
export default function Header({ view = 'workspace', onNavigate }: { view?: AppView; onNavigate?: (view: AppView) => void } = {}) {
  ...
  return (
    <>
      <header data-no-drag-select className={`safe-area-top fixed top-0 left-0 right-0 z-[60] bg-white/80 dark:bg-gray-950/80 backdrop-blur border-b border-gray-200 dark:border-white/[0.08] transition-transform duration-300 ease-in-out ${appMode === 'agent' && !agentMobileHeaderVisible ? '-translate-y-full sm:translate-y-0' : 'translate-y-0'}`}>
        <div className="safe-area-x safe-header-inner flex w-full max-w-none items-center justify-between relative">
